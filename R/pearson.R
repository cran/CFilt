pearson <- function(CF, type = c("user", "item"), i, j) {
  ...
  if(length(j)==1){
    if(type=="user"){
      ...
      avalia_v = sum(!is.na(v))
      avalia_w = sum(!is.na(w))

      #verificando se sao iguais
      if(
        sum(w==v,na.rm = T) == avalia_v &
        sum(w==v,na.rm = T) == avalia_w){
        s = 1
      } else {
        s = ifelse(
          sum(v*v,na.rm=T)==0 | sum(w*w,na.rm=T)==0,
          0,
  ...
  # Multiple comparisons ----
    ...
    if(type=="user"){
      ...
      if(type=="item"){
        ...
        s = apply(W_centered, MARGIN = 2, function(w){
